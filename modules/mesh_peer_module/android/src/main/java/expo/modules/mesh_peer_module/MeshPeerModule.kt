...
class MeshPeerModule : Module(), NearbyService.NearbyServiceListener {
    ...
    override fun definition() = ModuleDefinition {
        ...
        OnCreate {
            val delay = ReminderNotificationWorker.millisecondsUntilReminder()
            if (delay > 0) {
        ...
        AsyncFunction("startDiscovery") { promise: Promise ->
            ...
            try {
                ...
                Logger.error(TAG, "startDiscoveryImmediate failed", e)
                    ...
                    task.addOnFailureListener { exception ->
                        if (exception is ResolvableApiException) {
                            try {
                                ...
                                Logger.error(TAG, "Error showing location settings dialog", sendEx)
                            }
                        }
                    }
                }
        ...
        AsyncFunction("getUsername") { promise: Promise ->
            try {
                ...
                val sharedPrefs =
                    context.getSharedPreferences("cruise_chat_prefs", android.content.Context.MODE_PRIVATE)
        ...
        AsyncFunction("setUsername") { username: String, promise: Promise ->
            try {
                if (username.length >= 20) {
                    promise.reject(
                        "SET_USERNAME_ERROR",
                        "Failed to set username: username is too long (max 20 characters)",
                        null
                    )
                    return@AsyncFunction
                }
        ...
        AsyncFunction("clearNotificationSubscriptions") { promise: Promise ->
            try {
                ...
                promise.reject(
                    ...
                    "Failed to clear notification subscriptions: ${e.message}",
                    e
                )
            }
        }
